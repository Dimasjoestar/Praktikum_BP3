...
interface OrderDao {
    ...
    @Query("SELECT SUM(total) FROM orders WHERE status = 'DONE'")
    fun getTotalRevenueFlow(): Flow<Double?>

    @Query("SELECT COUNT(*) FROM orders WHERE status NOT IN ('DONE', 'DELIVERED', 'CANCELLED') AND createdAt < :orderCreatedAt")
    fun getQueueCountBeforeFlow(orderCreatedAt: Long): Flow<Int>
}
